...
// ===                   GLOBAL DECLARATIONS                    === 
// ================================================================
// struct config_fona_t config_fona;
// struct state_fona_t state_fona;


char replybuffer[255];   				// holds response from Fona
HardwareSerial *fonaSerial = &Serial1; 	// used to communicate with Fona board

Adafruit_FONA      fona = Adafruit_FONA(FONA_RST);		// Use this for FONA 800 and 808s
//Adafruit_FONA_3G fona = Adafruit_FONA_3G(FONA_RST);	// Use this one for FONA 3G (not yet supported by Loom)
...
// ================================================================
//
// Runs any startup for fona that should occur on device startup
// 
...
bool setup_fona() 
{
	...
	// not sure if I have to leave this a 115200, 
	// but will try just using the default 9600 that Loom normally uses, 
	// and then perhaps change everything to 11500 after if everything supports it
	// Serial.begin(115200); 
	...
	// --- The below comment is taken from the "FonaTest" example ---
	...
	// fona.setHTTPSRedirect(true);										// This might allow us to access Google Sheets without going through PushingBox
																		// Though I get 606 error when making HTTP requests with this enabled 
																		// I'll leave this off for now, this might be fixable later by moving to the Botletics
...
// ================================================================
//
// This is the testing loop and associated functions 
// from the 'fonatest' example code
//
...
void printMenu(void) 
{
	...
	// Exit Fona Test Loop
	Serial.println(F("[Q] Exit Fona Test Loop"));
	Serial.println(F("-------------------------------------"));
	Serial.println(F(""));
}
...
void fona_test_loop() 
{
	...
	switch (command) {
		...
		case 'Q': {
			Serial.println(F("Exiting Fona Test Loop"));
			return;
		}
