...
#define is_pushingbox 0		// 1 to enable PushingBox (Auto enabled if using LoRa hub) 
...
#define heatduration  20000
  
  #if probe_type == 0
    #define CUSTOM_NAME      "TDM"  // This is only used if Auto_name is set to be 0
	#elif probe_type == 1
    #define CUSTOM_NAME      "HRM"  // This is only used if Auto_name is set to be 0
  #endif // of probe_type
...
//  char device_id[]   = "v4099B684340616E";
	...
	#define useHubTabID 0
		...
		#define tab_id_complete "hubtest"    // Defines tab if hub is defining tab instead of bundle source
	#else
		// Use bundle source and below prefix to define tab ID
		#define tab_id_prefix   ""		// Used as a prefix if bundle source is being used to define tab
	#endif	

	#define verify_family_match 0
