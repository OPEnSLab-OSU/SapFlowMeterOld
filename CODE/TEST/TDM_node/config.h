...
// TDM
	...
	#define RF95_FREQ      915.0		// Hardware specific, Tx must match Rx
		...
		#define NODE_ADDRESS 6			// 10 CLIENT_ADDRESSes belong to each SERVER_ADDRESS, 
...
//    #define heatduration  240000   // Heating time 2500(=2.5s), 60000(=1m), 300000ms(=5m), 600000ms(=10m), 900000ms(=15m)
  	#define senddelay     60000 // send data every 2500(=2.5s), 60000(=1m), 300000ms(=5m), 600000ms(=10m), 900000ms(=15m)
