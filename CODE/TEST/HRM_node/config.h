...
#define CHANNEL          3	// Channel to use. Set to 1-8 for channels A-H, respectively. Alternatively can define to -1 to used advanced option INIT_INST
...
#define dynamic_serial_output   1// NOTE: 0: serial monitor must be opened for device to setup
...
// --- Set Device Name ---
	...
	#define DEVICE "HRM" // The device name (can be changed), used only if not using automatic device name
#endif
...
// HRM
	...
	#define RF95_FREQ      915.0		// Hardware specific, Tx must match Rx
		...
		#define NODE_ADDRESS 3			// 10 CLIENT_ADDRESSes belong to each SERVER_ADDRESS, 
...
// --- Sapflowmeter Options ---
	...
	#define is_rtc          0
	#define is_sd           1
  #define transmit_check  0         //enable check whether LoRa working

  #if transmit_check == 1
    #define checkdelay    100000  // 10min
  #endif
    ...
    #define probe_type    1      // 0:TDM, 1: HRM, 2:test
...
//    #define heatduration  120000   // Heating time 2500(=2.5s), 60000(=1m), 300000ms(=5m), 600000ms(=10m), 900000ms(=15m)
